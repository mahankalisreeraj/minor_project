...
class GoogleSheetsDB {
  ...
  async createProduct(productData) {
    ...
    const row = await sheet.addRow({
      ...
      // ✅ MULTILINGUAL FIELDS - Add empty values if not provided
      ...
      description_mr: productData.description_mr || '',
      description_ta: productData.description_ta || '',
      description_ur: productData.description_ur || ''
    });
    return row;
  }
  ...
  async getAllProducts() {
    try {
      ...
      if (rows.length > 0) {
        // Debug first row to see what fields are available
        const firstRow = rows[0];
        console.log('🔍 FIRST ROW DEBUG:');
        console.log('  Available row keys:', Object.keys(firstRow._rawData || {}));
        console.log('  Header values:', sheet.headerValues);
        
        // Test accessing multilingual fields
        console.log('🌍 MULTILINGUAL FIELD TEST:');
        console.log('  name:', firstRow.get('name'));
        console.log('  name_en:', firstRow.get('name_en'));
        console.log('  name_te:', firstRow.get('name_te'));
        console.log('  name_hi:', firstRow.get('name_hi'));
        console.log('  description:', firstRow.get('description'));
        console.log('  description_en:', firstRow.get('description_en'));
        console.log('  description_te:', firstRow.get('description_te'));
        console.log('  description_hi:', firstRow.get('description_hi'));
        
        // Check if fields exist in _rawData
        if (firstRow._rawData) {
          console.log('🔍 RAW DATA FIELDS:');
          Object.keys(firstRow._rawData).forEach(key => {
            if (key.includes('name') || key.includes('description')) {
              console.log(`  ${key}: "${firstRow._rawData[key]}"`);
            }
          });
        }
      }
